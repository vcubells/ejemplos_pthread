...
void * hilo2(void * null)
{
	...
	while(1)
	{
		...
		if (sig == SIGUSR1)
		{
			printf("Soy el hilo 2 y acabo de recibir la señal SIGUSR1 ...\n");
		}
	}
...
void * hilo1 (void * arg)
{
	...
	while(1)
	{
        ...
        if (signo == SIGINT)
		{
			...
			if (contador < 4)
			{
				printf("Soy el hilo 1 y acabo de recibir la señal SIGINT número %d  \n", contador);
				
                /* Enviar la señal SIGUSR1 al hilo 2 cuyo id se recibió como parámetro */
...
int main()
{
	...
	// Crear los hilos. Al hilo 1 se le pasa el tid del hilo 2 para que le pueda enviar la señal
